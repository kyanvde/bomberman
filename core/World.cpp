...
void World::movePlayer(const Vector2& direction, const float deltaTime) const {
    ...
    const bool canMoveX =
        insideWorldBounds(movedX, size) && !collidesWithBlockingEntity(currentPlayerIndex, movedX, size);

    const Vector2 movedY(nextPosition.x, nextPosition.y + delta.y);
    const bool canMoveY =
        insideWorldBounds(movedY, size) && !collidesWithBlockingEntity(currentPlayerIndex, movedY, size);
