...
void GameOverState::layout(const sf::Vector2u& size) {
    ...
    resultText.setPosition(static_cast<float>(size.x) * 0.5f, static_cast<float>(size.y) * 0.4f);
    ...
    hintText.setPosition(static_cast<float>(size.x) * 0.5f, static_cast<float>(size.y) * 0.6f);
}
