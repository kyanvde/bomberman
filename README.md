...
The three inheritance hierarchies the design is built around, generated by Doxygen/Graphviz from the source:

**Entities** — one `EntityModel` base (itself a `Subject`, so every entity can notify its observers) with a
concrete subclass per game object:

![Entity class hierarchy](docs/diagrams/class-hierarchy-entities.png)

**Observers** — the `Observer` interface has two independent kinds of implementer: the per-entity `EntityView`s
that redraw, and `Score`, which reacts to the very same events for scoring:

![Observer class hierarchy](docs/diagrams/class-hierarchy-observers.png)

**States** — the screen flow, as a stack of `State` objects:

![State class hierarchy](docs/diagrams/class-hierarchy-states.png)

Full API documentation, including a UML class diagram with members and relations for *every* class, is generated
into `docs/html/` — open `docs/html/index.html` (or `docs/html/inherits.html` for the complete hierarchy). Rebuild
it with `cmake --build build --target documentation`.
